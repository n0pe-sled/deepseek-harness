...
- `sidebar`: one of the `DocsSidebar` collections in `sidebarCollections` (`guide`, `develop`, `reference`), or `null` for the site home page.
- `section`: reuse an existing section when possible. If adding one, also add it to the `sections` list in [website/docs.ts](../../../website/docs.ts); `sectionSpec()` throws on an unplaced label.
