...
describe('web e2e: settings modal and General preferences', () => {
  ...
  beforeAll(async () => {
    ...
    // English browser: the shared page asserts the shipped settings surface.
  ...
  it('opens the settings dialog, switches sections, and closes by every path', async () => {
    ...
    // Golden of the freshly opened dialog (default en, General active).
  ...
  it('opens an English browser in English without any stored preference', async () => {
    ...
    // distinguish detection from the default.
  ...
  it('opens a browser asking for no shipped language in English', async () => {
    // The product default for "no usable signal": a French browser names no
    // shipped language, so resolution falls to FALLBACK_LOCALE (en).
    const fresh = await launchWebScaffold({})
