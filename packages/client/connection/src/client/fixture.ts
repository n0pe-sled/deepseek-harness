...
const USER_MARKDOWN_LITERAL = 'User literal: # does not render `code` [link](https://example.com)'
...
function buildAlphaLog(): SessionEvent[] {
  ...
  for (let turn = 0; turn < 60; turn++) {
    ...
    const userSeq = push({
      ...
      data: userMessage(text(turn === 59 ? USER_MARKDOWN_LITERAL : `Question ${turn}: fixture history message for pagination and render checks.`)),
    })
    if (turn === 0) {
      push({
        type: 'session/title',
    ...
    if (withReasoning) blocks.push({ type: 'reasoning', text: `Reasoning ${turn}: a collapsible reasoning passage.` })
    blocks.push({ type: 'text', text: turn === 59 ? MARKDOWN_FIXTURE : `Answer ${turn}: fixture-generated assistant reply body.` })
    if (withTool) {
      ...
      push({ type: 'assistant/message', surfaceOp: 'append', data: { turn, step: 1, message: assistantMessage(text(`Tool result consumed (turn ${turn}).`)) } })
  ...
  toolTurn(62, 'edit', '{"file_path":"notes/demo.txt","old_string":"hello","new_string":"hello fixture"}', 'edited')
  ...
  toolTurn(64, 'edit', '{"file_path":"src/config.ts","old_string":"const timeout = 30","new_string":"const timeout = 60"}', 'edited')
  ...
  const fixtureTodos = [
    ...
    { content: 'Implement fixture sample', status: 'in_progress' },
  ...
  push({
    ...
    surfaceOp: 'append',
    data: { turn: 72, step: 0, message: assistantMessage(text('Entry 1: first one. Entry 2: second one. Entry 3: this one stops mid-')) },
  })
...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const fixtureQuestions: Extract<MuxFrame, { type: 'question/requested' }>['questions'] = [
    {
      id: 'harness-profile',
      header: 'Preference',
      question: 'Which kind of Agent/Harness candidate do you want to hire now?',
      options: [
        { label: 'Engineering-focused (Recommended)', description: 'Values hands-on work on runtime, tool executor, sandbox, trace, and diagnosing production issues.' },
        { label: 'Research potential', description: 'Values Agent understanding, training/evaluation ideas, and long-term growth.' },
        { label: 'Balanced', description: 'Requires both engineering ability and Agent insight, but may raise the screening bar.' },
      ],
    },
    {
      id: 'work-mode',
      ...
      options: [
        { label: 'Build a small prototype first (Recommended)', description: 'Validate key assumptions quickly with a runnable result.' },
  ...
  const timingHooks = {
    ...
    beginModelRetry(id: string): void {
      ...
      append(sessionId, { type: 'assistant/chunk', data: { turn, step: 1, chunk: { type: 'text-delta', index: 0, text: 'Partial reply that should be retracted' } } })
    },
    ...
    scheduleModelRetry(id: string, retry = 1, delayMs = 450): void {
      ...
      if (!scenario.stepStarted) {
        ...
        append(sessionId, { type: 'assistant/chunk', data: { turn: scenario.turn, step: 1, chunk: { type: 'text-delta', index: 0, text: `Reply to retract on attempt ${String(retry)}` } } })
    ...
    completeModelRetry(id: string): void {
      ...
      append(sessionId, {
        ...
        data: {
          turn: scenario.turn,
          step: 1,
          message: assistantMessage(text('Complete reply after retry')),
        },
      })
      append(sessionId, { type: 'step/end', data: { turn: scenario.turn, step: 1 } })
  ...
  const api: ApiProxy = {
        ...
        startReply(
          ...
          userText === 'render markdown'
            ...
            : userText === 'report model'
              ...
              : `Echo: ${userText}. This is the fixture streaming reply, used to verify typewriter growth and finalize switching.`,
        )
        return ok(request, { accepted: true as const })
      },
      attachment: (request) => {
    ...
    skills: {
      list: (request) => {
        ...
        return ok(request, {
          skills: [
            { name: 'fixture-demo', description: 'fixture skill sample', whenToUse: 'For UI catalog render checks only', modelInvocable: true },
        ...
        if (approvalPending) {
          conn.push({
            ...
            payload: {
              ...
              toolName: 'dangerous_tool', reason: 'fixture resident approval (answer it: approve/reject makes it disappear)',
            },
          })
        }
        if (questionPending) {
