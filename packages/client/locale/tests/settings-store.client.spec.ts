...
const OPTIONS = [{ id: 'en', label: 'English' }]
...
describe('createLanguageRowStore', () => {
  ...
  it('sync mirrors the snapshot and advances the revision', () => {
    const store = createLanguageRowStore().create()
    store.actions.sync('en', OPTIONS, 0)
    expect(store.getSnapshot()).toEqual({ active: 'en', options: OPTIONS, revision: 0 })
    store.actions.sync('fr', OPTIONS, 1)
    expect(store.getSnapshot().active).toBe('fr')
