  ...
  it('marks a broken custom preset: unselectable, uncopyable, still deletable', () => {
    const actions = renderSection({
      rows: [
        ...
        { id: 'ghost', trust: 'user', isDefault: false, name: 'Ghost preset', broken: 'the composition file agent.cordis.yml is missing' },
      ],
    })
...
describe('a long card description', () => {
  ...
  const LONG = 'A friendly general-purpose assistant that always replies in English, with persistent bash and file editing.'.repeat(8)
  ...
  it('offers the whole description on hover once the card cuts it off', () => {
    ...
    vi.useFakeTimers()
    try {
      renderSection({ rows: [{ id: 'zh', trust: 'user', isDefault: false, name: 'English assistant', description: LONG }] })
  ...
  it('stays quiet when the description already fits', () => {
    ...
    vi.useFakeTimers()
    try {
      renderSection({ rows: [{ id: 'zh', trust: 'user', isDefault: false, name: 'English assistant', description: 'Short description.' }] })

      fireEvent.mouseEnter(within(rowFor('zh')).getByText('Short description.'))
      act(() => { vi.advanceTimersByTime(400) })
