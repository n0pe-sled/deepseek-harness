...
describe('ChatView', () => {
  ...
  it('renders terminal turn failures inline with their durable message and optional code', () => {
    ...
    expect(statuses.map(status => status.textContent)).toEqual([
      'This turn failed API key is invalidAUTH',
      'This turn failed plugin exploded',
    ])
  })
  ...
  it('renders the max-tokens notice with localized guidance, distinct from turn errors', () => {
    ...
    const statuses = view.getAllByRole('status')
    expect(statuses.map(status => status.textContent)).toEqual([
      'Output token limit reached The reply was cut off; earlier output is preserved in the conversation. Send "continue" to let the model resume.',
    ])
  ...
  it('names a workspace-folder Host refusal as a folder', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole('dialog', { name: 'Couldn’t open folder' }).textContent).toContain('Couldn’t open this folder')
    })
  })
