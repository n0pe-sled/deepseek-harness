...
const COMMANDS: FakeCommand[] = [
  { name: 'goal', description: 'set goal', input: { hint: 'goal content' } },
  { name: 'compact', description: 'compact context' },
  { name: 'vision', description: 'recognize image', input: { hint: 'ask what', images: true } },
]
...
async function bench(executeImpl?: (line: string) => Promise<SubmitOutcome>) {
  const execute = vi.fn(executeImpl ?? ((line: string) =>
    Promise.resolve({ kind: 'success' as const, text: `ran ${line}` })))
...
describe('scenario A: menu-pick /goal, type args, enter submits', () => {
  it('runs the whole claim chain through the real pipeline', async () => {
    ...
    await vi.waitFor(() => { expect(b.execute).toHaveBeenCalledWith('/goal ship v1', []) })
    await vi.waitFor(() => { expect(b.textarea.value).toBe('') })
    expect(b.shell.snapshot.phase).toBe('plain')
    expect(b.view.getByText('ran /goal ship v1')).toBeTruthy()
...
describe('scenario C: pasted /goal xxx + enter (menu never opened)', () => {
  it('adjudicates on enter, claims and submits in one stroke', async () => {
    ...
    act(() => { b.shell.setDraft('/goal ship soon') })
    fireEvent.keyDown(b.textarea, { key: 'Enter' })
    await vi.waitFor(() => { expect(b.execute).toHaveBeenCalledWith('/goal ship soon', []) })
...
describe('scenario: images ride an accepting command through the real pipeline', () => {
  it('adjudication reports the image count; the claim chain serializes, submits, and consumes', async () => {
    ...
    act(() => { b.shell.setDraft('/vision what is this image') })
    fireEvent.keyDown(b.textarea, { key: 'Enter' })
    await vi.waitFor(() => { expect(b.execute).toHaveBeenCalledWith('/vision what is this image', [PNG]) })
  ...
  it('an imageless enter adjudicates with a zero-image envelope', async () => {
    const b = await bench()
    act(() => { b.shell.setDraft('/goal ship') })
    fireEvent.keyDown(b.textarea, { key: 'Enter' })
    await vi.waitFor(() => { expect(b.execute).toHaveBeenCalledWith('/goal ship', []) })
  ...
  it('adjudication failure (source warmup throw) notices and keeps the draft', async () => {
    const b = await scopedBench((inputTriggers) => {
      inputTriggers.registerSource({
        ...
        candidates: () => Promise.resolve([]),
        onPick: () => undefined,
        matchEnter: () => Promise.reject(new Error('directory prewarm failed')),
      } as never)
    })
    act(() => { b.shell.setDraft('/plan launch') })
    fireEvent.keyDown(b.textarea, { key: 'Enter' })
    await vi.waitFor(() => { expect(b.view.getByText('directory prewarm failed')).toBeTruthy() })
