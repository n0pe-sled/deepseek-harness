...
interface SearchBlockCommon {
   ...
   * folds the pre-cap `total` in (`Showing X / N total …`) so the card never presents a
...
/**
 * The banner summary. When the search was capped it reads `Showing X / N total …` so
...
function summaryText(props: SearchBlockProps, shown: number, truncated: boolean, total: number): string {
  const count = truncated ? `Showing ${shown} / ${total} total` : `${shown}`
  return props.kind === 'paths'
    ? `${count} paths`
          ...
          <div className={css.body}>
            ...
            {hidden > 0 && (
              <button
                ...
                aria-label={expanded ? 'Collapse results' : `Show ${hidden} more result lines`}
                onClick={onToggle}
              >
