...
describe('SearchBlock matches kind', () => {
  it('renders each file as a header group with its matched lines', () => {
    ...
    expect(view.queryByText(/Showing|total/u)).toBeNull()
  })
  ...
  it('folds the pre-cap total into the summary when truncated', () => {
    ...
    // `summaryText` has no plural arm for the file unit, so a single file still
    // renders as `1 files`.
    expect(view.getByText('Showing 2 / 99 total matches · 1 files')).toBeTruthy()
  })
})
...
describe('SearchBlock paths kind', () => {
  ...
  it('folds the pre-cap total into the paths summary when truncated', () => {
    ...
    expect(view.getByText('Showing 2 / 50 total paths')).toBeTruthy()
  })
})
...
describe('SearchBlock height cap', () => {
  it('renders every row and no expand control under the cap', () => {
    ...
    expect(view.container.querySelector('[aria-label^="Show"]')).toBeNull()
  })
  ...
  it('slices head and tail over the cap and expands on click', () => {
    ...
    const toggle = view.getByRole('button', { name: 'Show 6 more result lines' })
  ...
  it('counts a file header as one capped row alongside its matches', () => {
    ...
    expect(view.getByRole('button', { name: 'Show 7 more result lines' })).toBeTruthy()
  })
  ...
  it('renders the head slice alone when the cap leaves no tail', () => {
    ...
    expect(view.getByRole('button', { name: 'Show 4 more result lines' })).toBeTruthy()
  })
  ...
  it('restores the owning file header above a tail slice that begins mid-file', () => {
    ...
    expect(view.getByRole('button', { name: 'Show 14 more result lines' })).toBeTruthy()
  })
  ...
  it('caps at the documented default when maxLines is absent', () => {
    ...
    expect(view.getByRole('button', { name: 'Show 1 more result lines' })).toBeTruthy()
  })
})
